...
| Marketing homepage design direction (Phase 5) | Photo-forward hero + card-style offering grid (structure borrowed from mainevent.com/daveandbusters.com), but reinterpreted in the brand's own collegiate/varsity direction rather than their arcade-nightlife palette — real logo, live facility data (ticker + stats) instead of generic marketing copy | User explicitly asked for something "engaging and fun" with animations/pictures/the real logo, after an initial flat/subdued concept was rejected as bland. Iterated through three mockups (flat cards → hover/motion → photo-forward cards) before landing here. Real facility photography does not exist yet — `apps/marketing/app/components/OfferingCards.tsx` uses gradient-tinted color blocks as a placeholder, flagged for a real photo shoot before launch, not guessed/hotlinked stock photo URLs (avoids depending on unverified external image URLs). |
...
- `GET /public/open-now` — current + next ~4h of schedule blocks across all spaces (space name, sport name, mode, times) — powers the homepage's live ticker. No participant or account data.
- `GET /public/stats` — three aggregates: `activeMembers` (count of `accounts` with `status = active`), `tokensInPlay` (`SUM(token_ledger.amount)` — total unspent tokens currently held across every participant, not a per-person balance), `leaguesRunning` (count of active `league`-type offerings).
