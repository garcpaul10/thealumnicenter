...
## 11. Public/marketing API surface (Phase 5)

`apps/api/src/routes/public.ts` — no auth, no auth middleware at all, registered before every other route group. Called directly from client-side JS in `apps/marketing` (`apps/marketing/lib/api.ts`), the same way `apps/scan-station` calls the API directly rather than proxying through Server Actions — there's no session to protect here, so there's nothing to hide from client JS.
...
- `GET /public/stats` — three aggregates: `activeMembers` (count of `accounts` with `status = active`), `tokensInPlay` (`SUM(token_ledger.amount)` — total unspent tokens currently held across every participant, not a per-person balance), `leaguesRunning` (count of active `league`-type offerings). Kept in the API even though the redesigned homepage no longer surfaces it (Apple's restraint-over-density direction dropped the raw stat row) — cheap to keep, and a future page (e.g. an "about" page) may still want it.
- `GET /public/sports` — active sports list (name/slug/icon), for any future sport-driven marketing content.

**Real facility photography does not exist yet.** `apps/marketing/app/components/OfferingCards.tsx` uses gradient-tinted color blocks in place of real photos — flagged here as a genuine pre-launch gap (a photo shoot or licensed stock), not something silently shipped as final. Deliberately not using hotlinked/guessed stock photo URLs, since an unverified external URL could 404 or resolve to the wrong image.
